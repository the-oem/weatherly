...
const WeatherFull = ({ state, toggleView }) => {
  ...
  return (
    <section className='weather-full'>
      <WeatherNow { ...now }/>
      {contentHourly}
      <button className='toggle-hourly' onClick={() => toggleView('showHourly')}>toggle hourly</button>
      {contentDaily}
      <button className='toggle-daily' onClick={() => toggleView('showDaily')}>toggle 10-day</button>
    </section>
    );
};
